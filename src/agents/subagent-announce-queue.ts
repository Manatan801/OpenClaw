...
function getAnnounceQueue(
  ...
  const created: AnnounceQueueState = {
    ...
    lastEnqueuedAt: 0,
    mode: settings.mode,
    debounceMs: typeof settings.debounceMs === "number" ? Math.max(0, settings.debounceMs) : 200,
    cap: typeof settings.cap === "number" && settings.cap > 0 ? Math.floor(settings.cap) : 100,
