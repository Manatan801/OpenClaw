...
const log = createSubsystemLogger("infra/unhandled");
  ...
  for (const handler of handlers) {
    try {
      ...
      log.error(
        ...
        err instanceof Error ? { error: err.stack ?? err.message } : { error: err },
      );
    }
  }
  return false;
...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      log.warn("[openclaw] Suppressed AbortError:", { error: formatUncaughtError(reason) });
      return;
    }

    if (isFatalError(reason)) {
      log.error("[openclaw] FATAL unhandled rejection:", { error: formatUncaughtError(reason) });
      process.exit(1);
      return;
    }

    if (isConfigError(reason)) {
      log.error("[openclaw] CONFIGURATION ERROR - requires fix:", {
    ...
    if (isTransientNetworkError(reason)) {
      log.warn("[openclaw] Non-fatal unhandled rejection (continuing):", {
        error: formatUncaughtError(reason),
      });
      return;
    }

    log.error("[openclaw] Unhandled promise rejection:", { error: formatUncaughtError(reason) });
    process.exit(1);
  });
}
