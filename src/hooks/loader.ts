 ...
 * console.log(`Loaded ${count} hook handlers`); // console-ok
 * ```
 */
export async function loadInternalHooks(
  ...
  try {
    ...
    for (const entry of eligible) {
      ...
      try {
        ...
        if (events.length === 0) {
          log.warn(`Hook warning: Hook '${entry.hook.name}' has no events defined in metadata`);
          continue;
        }
        ...
        log.error(`Failed to load hook ${entry.hook.name}:`, {
          error: err instanceof Error ? err.message : String(err),
        });
      }
    }
  } catch (err) {
    log.error("Failed to load directory-based hooks:", {
      error: err instanceof Error ? err.message : String(err),
    });
  }
  ...
  for (const handlerConfig of handlers) {
    try {
      ...
      if (typeof handler !== "function") {
        log.error(`Hook error: Handler '${exportName}' from ${modulePath} is not a function`);
        continue;
      }
      ...
      log.error(`Failed to load hook handler from ${handlerConfig.module}:`, {
        error: err instanceof Error ? err.message : String(err),
      });
    }
  }
